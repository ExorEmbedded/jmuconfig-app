...
(
    ...
    ./start.sh $@;
    if [ -z "$FASTBOOT" ]; then
        dbus-send --print-reply --system --dest=com.exor.JMLauncher '/' com.exor.JMLauncher.appFinished string:"HMI Client" 
    fi
...
if ( pgrep HMI > /dev/null ); then
    ...
    if [ -z "$FASTBOOT" ]; then
        dbus-send --print-reply --system --dest=com.exor.JMLauncher '/' com.exor.JMLauncher.appLoaded string:"HMI Client"
    fi
fi
